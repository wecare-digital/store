...
async function collectExistingSkus ( products ) {
  ...
  for ( const p of products ) {
    ...
    for ( const v of full.variantsInfo?.variants || [] ) if ( v.sku && v.sku.trim() ) used.add( v.sku.trim().toUpperCase() );
  }
  return used;
}
    ...
    for ( const p of products ) {
      try {
        ...
        const result = await setVariantSkus( p.id, ( variant, i, variants ) => {
          ...
          if ( !candidates ) {
            for ( const v of variants ) if ( ( v.sku || '' ).trim() ) usedSkus.add( v.sku.trim().toUpperCase() );
        ...
        } else skipped.push( { id: p.id, name: p.name, reason: 'all variants already have SKUs' } );
      } catch ( err ) { errors.push( { id: p.id, name: p.name, error: err?.message || String( err ) } ); }
    }
    return {
      dryRun, catalogVersion: 'V3', prefix: cleanPrefix || null, totalProducts: products.length,
      updated, skipped, errors, variantsUpdated,
...
export const reprefixSKUs = webMethod(
  ...
  async ( { oldPrefix = '', newPrefix = 'WD', dryRun = true } = {} ) => {
    ...
    for ( const p of products ) {
      try {
        const result = await setVariantSkus( p.id, ( variant, index, variants ) => {
          if ( index === 0 ) for ( const v of variants ) if ( ( v.sku || '' ).trim() ) usedSkus.add( v.sku.trim().toUpperCase() );
          const current = ( variant.sku || '' ).trim();
    ...
    return {
      dryRun, catalogVersion: 'V3', oldPrefix: cleanOld || null, newPrefix: cleanNew,
      totalProducts: products.length, updated, skipped, errors,
...
export const assignMissingSKUs = webMethod(
  Permissions.Admin,
  async ( { prefix = 'WD', dryRun = true } = {} ) => assignSkusToAllProducts( { overwrite: false, dryRun, prefix } )
);
