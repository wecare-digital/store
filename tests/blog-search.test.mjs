...
function fixture({ missing = [], globalSelector = true, mobile = false } = {}) {
  ...
  const wixData = {
    query(collection) {
      ...
      const chain = {
        contains(field, value) { request.field = field; request.value = value; return chain; },
        ascending(field) { request.sort = field; return chain; },
  ...
  const ctx = vm.createContext({
    wixData, wixWindow: { formFactor: mobile ? 'Mobile' : 'Desktop' }, wixLocation: { to: url => visits.push(url) },
    resolveBlogPostUrl, console: { error: (...x) => errors.push(x), warn: (...x) => errors.push(x) },
    setTimeout: fn => { timers.set(++nextTimer, fn); return nextTimer; },
    clearTimeout: id => timers.delete(id), ...(globalSelector ? { $w: select } : {}),
  });
  ...
  return {
    ...
    async resolve(index, items) { requests[index].resolve({ items }); await flush(); },
    async reject(index) { requests[index].reject(new Error('Query unavailable')); await flush(); },
  };
}

const resentment = { _id: 'resentment-id', title: 'Resentment', slug: 'resentment' };
const flame = { _id: 'flame-id', title: 'Flame', slug: 'flame' };
...
test('Enter while a new query is pending never opens old results', async () => {
  const f = fixture(); f.init(); f.type('Resentment'); f.runTimers(); await f.resolve(0, [resentment]);
  f.type('Flame'); f.runTimers(); f.enter();
  assert.deepEqual(f.visits, []);
  await f.resolve(f.requests.length - 1, [flame]);
  assert.deepEqual(f.visits, ['/post/flame']);
});
...
test('typing shows clickable results using the documented Blog collection query', async () => {
  const f = fixture(); f.init(); f.type('Resentment'); f.runTimers();
  assert.equal(f.requests[0].collection, 'Blog/Posts');
  assert.equal(f.requests[0].field, 'title');
  assert.equal(f.requests[0].value, 'Resentment');
  await f.resolve(0, [resentment]);
...
test('a current query failure is visible and distinguishable from no matches', async () => {
...
test('mobile results preserve the touch-row sizing and scroll into view', async () => {
