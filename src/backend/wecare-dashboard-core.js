...
<section id="t4" class="panel"><div class="card"><div class="row"><h2>Invoices</h2><button class="btn primary" onclick="loadInvoices()">Refresh</button><a class="btn" target="_blank" href="${links.invoices}">Open Invoices</a></div><div id="invoicesOut" class="muted">Load invoices to begin.</div></div></section>
<section id="t5" class="panel"><div class="card"><div class="row"><h2>Payment Links</h2><button class="btn primary" onclick="loadPaymentLinks()">Refresh</button><a class="btn" target="_blank" href="${links.payLinks}">Open Payment Links</a></div><div id="paymentLinksOut" class="muted">Load payment links to begin.</div></div></section>
<section id="t6" class="panel"><div class="card"><div class="row"><h2>Forms</h2><button class="btn primary" onclick="loadForms()">Refresh</button><a class="btn" target="_blank" href="${links.forms}">Open Forms</a></div><div id="formsOut" class="muted">Load Wix Forms to begin.</div></div></section>
...
<section id="t9" class="panel"><div class="card"><h2>WhatsApp</h2><div id="whatsappOut" class="muted">Checking connection…</div><h3>Order activity mapping</h3><ul><li>Order approved → confirmation + custom WECARE Order ID</li><li>Payment status updated → payment update</li><li>Order fulfilled → fulfillment/delivery update</li><li>Refund completed → refund update</li></ul><p class="muted">Meta app 2238810740192680 is assigned. Outbound messaging stays disabled until WABA, Phone Number ID, and access-token secrets are present.</p></div></section>
...
async function loadInvoices(){const el=document.getElementById('invoicesOut');el.textContent='Loading…';try{const d=await call('invoices');el.innerHTML=table(d.invoices,[['Invoice','number'],['Title','title'],['Status','status'],['Created','createdDate'],['Due','dueDate'],['Total','total']]);}catch(e){el.textContent=e.message;}}
async function loadPaymentLinks(){const el=document.getElementById('paymentLinksOut');el.textContent='Loading…';try{const d=await call('paymentLinks');el.innerHTML=table(d.paymentLinks,[['Title','title'],['Status','status'],['Type','type'],['Currency','currency'],['Created','createdDate'],['URL','url']]);}catch(e){el.textContent=e.message;}}
async function loadForms(){const el=document.getElementById('formsOut');el.textContent='Loading…';try{const d=await call('forms');el.innerHTML=table(d.forms,[['Form','name'],['Enabled','enabled'],['Updated','updatedDate'],['Namespace','namespace']]);}catch(e){el.textContent=e.message;}}
